...
Once the CLI is set up, you can begin using it to interact with the Mint Client plugin system. Below are some useful commands you can use:
...
- To download a template of existing plugin for development:\
  ...
  ```
  <Aside type="danger">Do not download plugin's you dont trust!</Aside>
- To create an empty plugin with a default example:\
  Replace `<output-folder>` with the desired folder name you want,
    and `<1.8.9 | 1.18.2 | 1.20.6>` with the version of the plugin you want to create.
  This creates a default plugin with example code.
  ```bash
